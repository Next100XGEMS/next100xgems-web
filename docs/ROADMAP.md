...
Radar signal gap closure — IN PROGRESS / DEVELOPMENT ONLY. Added read-only Pump bonding-curve/PDA decoding, exact holder concentration, conflict taxonomy, conservative EVM semantic assessment and structural historical-outcome labels. Solana remains MORE_DATA_WORK_REQUIRED; BNB/Base remain shadow; no scoring, AI, production provider, public activation or trading/execution was started. Remaining evidence gaps require a reviewed historical corpus and lifecycle/pool/holder collection before methodology design.

Final Solana data validation + historical corpus — REAL CORPUS ACQUIRED / METHODOLOGY GATED. Acquired and froze 300 unique Pump mints from a 90-day window using deterministic 13-bucket oldest/newest sampling, with current lifecycle enrichment, creator/bonding-curve provenance and 1,800 identity-only checkpoint manifests. Market/holder as-of history, PumpSwap migration linkage and future outcome labels remain incomplete; Solana remains MORE_DATA_WORK_REQUIRED and Fast Lane methodology design remains gated.

Radar PumpSwap linkage + prospective shadow collection — DEVELOPMENT READY / METHODOLOGY DATA PENDING. Added an official read-only Pump `migrate`/`migrate_v2` to canonical PumpSwap Pool decoder and a restart-safe, bounded shadow runner for frozen Solana admissions, T+5M through T+24H checkpoints, explicit PENDING_FUTURE/DUE_NOW/CAPTURED/MISSED_WINDOW states, lifecycle-aware market stages, market fallback, provenance and cost telemetry. The authoritative cohort is 73 unique admissions with 438 scheduled checkpoints: 44 captured, 321 missed windows and 73 future. The local watcher resumes persisted state and stops automatically when T+24H is terminal. `SHADOW_COLLECTION_READY` is implementation-ready; `METHODOLOGY_DATA_READY` remains false until later checkpoints and objective future labels mature. No scoring, AI, public activation, production scheduling or trading/execution was added. See [RADAR_SHADOW_COLLECTION.md](RADAR_SHADOW_COLLECTION.md).
...
Universal Token Analyzer deterministic intelligence — INTERNAL DEVELOPMENT
IMPLEMENTATION. Connected selective read-only Solana (Helius/Solana RPC,
Pump/PumpSwap, Birdeye, DEX Screener) and EVM (Alchemy plus DEX Screener)
observation paths for bounded Admin smoke testing. CoinGecko and GMGN remain
capability-advertised selective/optional enrichment. The master and public
flags remain OFF by default, AI and methodology remain inactive, and the
Analyzer does not publish, score, or execute trades. Social/article live
analysis is deliberately a later phase.
...
Authenticated Multi-Chain Provider Acceptance Pass — PARTIAL / MEASURED 2026-09-19. The exact frozen cohort was reused after all six requested credential names became locally available. Helius direct Solana reads, Birdeye price/holder routes, public DEX comparison, CoinGecko Demo authentication, and Alchemy read-only endpoint access were measured; GMGN was not called because no approved general read-only data route was available. Helius and Birdeye are usable for development/shadow collection only with pacing, caching and candidate-only enrichment; CoinGecko Demo and Base/BNB Alchemy access returned authentication/permission failures. No paid upgrade, production chain promotion, AI, trading or execution activation was made.

Radar AI Evaluation — bake-off specification COMPLETE; experiments NOT RUN. [RADAR_AI_EVALUATION.md](RADAR_AI_EVALUATION.md) defines current proprietary/open-weight candidates, separate routine/escalation/adjudication/social/shadow roles, a proposed 400-case frozen-evidence dataset, privacy/license gates and cost scenarios. The core remains frozen manifest → bounded model → structured output → validation; optional research-agent tooling cannot replace durable Deep Lane authority. Next: human approval of universe, accounts, read-only keys, subscriptions, data/model-input rights, experimental budgets, hosting and reviewers, then separately authorized integration/shadow evaluation. No provider/model was selected for production, purchased or integrated; no credential, scoring/freshness policy, deployment or runtime change was introduced.
