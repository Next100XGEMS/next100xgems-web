...
Radar PumpSwap linkage + prospective shadow collection — DEVELOPMENT READY / METHODOLOGY DATA PENDING. Added an official read-only Pump `migrate`/`migrate_v2` to canonical PumpSwap Pool decoder and a restart-safe, bounded shadow runner for real Solana admissions, T+5M through T+24H checkpoints, exact observations, holder/top-account context, market fallback, provenance and cost telemetry. A bounded live validation linked 7/10 committed completed cases; the bootstrap admitted 20 real Pump tokens and captured 14 T+5M checkpoints. `SHADOW_COLLECTION_READY` is the implementation target; `METHODOLOGY_DATA_READY` remains false until checkpoints and objective future labels mature. No scoring, AI, public activation, production scheduling or trading/execution was added. See [RADAR_SHADOW_COLLECTION.md](RADAR_SHADOW_COLLECTION.md).
