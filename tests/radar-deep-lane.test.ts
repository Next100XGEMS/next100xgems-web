...
const request = prepareDeepLaneRequest({
...
describe("Radar provider-independent Deep Lane", () => {
  ...
  it("rejects a request mutated after its identity was sealed", () => { const mutated = { ...request, methodVersion: "changed-method" }; expect(() => validateDeepLaneOutput({ schemaVersion: "deep-lane-output-v1", provider: "fixture", model: "fixture-model", modelRevision: "fixture-v1", requestHash: request.requestHash, generatedAt: "2026-09-19T00:00:00.000Z", inferences: [] }, mutated)).toThrow(/changed after sealing/); });
  ...
  it("uses one durable reservation and one provider idempotency key", async () => { const adapter = new FixtureDeepLaneProvider(); const active = gateway(); const result = await executeDeepLaneWork({ claimed, request, adapter, gateway: active }); expect(result.requestHash).toBe(request.requestHash); expect(adapter.invocationCount).toBe(1); expect(adapter.invocationKeys).toEqual(["deep-lane-v1:key-1"]); expect(active.reserveDeepLaneAttempt).toHaveBeenCalledOnce(); });
  it("does not invoke the adapter for an in-progress duplicate or completed replay", async () => {
    ...
    const completed = executeDeepLaneWork({ claimed, request, adapter, gateway: gateway({ reserveDeepLaneAttempt: vi.fn().mockResolvedValue(reservation({ requestState: "SUCCEEDED", attemptState: "SUCCEEDED", invocationOwner: false, completionOutput: { ...completedOutput, outputHash: undefined }, completionOutputHash: completedOutput.outputHash })) }) });
