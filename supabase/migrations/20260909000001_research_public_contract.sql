...
-- Gate 18F2: keep the existing Research schema and security model, but make
-- every newly saved/published value fit the public presentation contract.
-- This migration is additive: historical private rows are not rewritten.
...
begin
  ...
  for s in select value from jsonb_array_elements(p_body->'sections') loop
    ...
    section_id := s->>'id';
    if length(section_id) not between 1 and 80 or length(s->>'heading') > 160
    ...
    for b in select value from jsonb_array_elements(s->'blocks') loop
      ...
      if b ? 'id' then
        ...
        block_id := b->>'id';
        if length(block_id) not between 1 and 80 or block_id = any(block_ids) then return false; end if;
      ...
      if b->>'type' = 'quote' and b ? 'attribution' and
        (jsonb_typeof(b->'attribution') is distinct from 'string' or length(b->>'attribution') > 240) then return false; end if;
      if b->>'type' = 'callout' and
        (jsonb_typeof(b->'label') is distinct from 'string' or length(btrim(b->>'label')) not between 1 and 120) then return false; end if;
      if b->>'type' = 'data_placeholder' and
        (jsonb_typeof(b->'label') is distinct from 'string' or length(btrim(b->>'label')) not between 1 and 160
          or jsonb_typeof(b->'description') is distinct from 'string' or length(b->>'description') > 10000) then return false; end if;
    end loop;
  end loop;
  return total <= 250;
end;
  ...
  if p_input ? 'sources' then
    for item in select value from jsonb_array_elements(p_input->'sources') loop
      if jsonb_typeof(item) <> 'object'
        ...
        or length(btrim(item->>'title')) not between 1 and 240
        or length(btrim(item->>'publisher')) not between 1 and 160 then
...
begin
  ...
  if not exists (select 1 from public.research_authors y where y.profile_id=a.public_author_id
      and length(btrim(y.display_name)) between 1 and 160
      and (y.title is null or length(btrim(y.title)) between 1 and 160))
    ...
    or exists (select 1 from public.article_sources where article_id=a.id and retired_at is null
      and (length(btrim(title)) not between 1 and 240 or length(btrim(publisher)) not between 1 and 160)) then
...
begin
  ...
  if p_display_name is null or length(btrim(p_display_name)) not between 1 and 160 or p_display_name ~ '[[:cntrl:]]'
    or (p_title is not null and (length(btrim(p_title)) not between 1 and 160 or p_title ~ '[[:cntrl:]]')) then
