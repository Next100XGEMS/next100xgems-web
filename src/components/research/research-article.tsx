...
function ArticleRelatedContent({ article }: { article: ResearchArticle }) {
  ...
  return <Section density="compact" className="border-t border-[var(--n100-border-subtle)]"><Container size="reading"><div className="grid gap-8 sm:grid-cols-2">{hasResearch ? <div><h2 className="text-lg font-semibold text-[var(--n100-text-primary)]">Related Research</h2><ul className="mt-4 space-y-3">{article.relatedResearch?.map((item) => <li key={item.slug}><Link href={`/research/${item.slug}`} className="text-sm text-[var(--n100-text-secondary)] underline decoration-[var(--n100-border-strong)] underline-offset-4 hover:text-[var(--n100-accent)]">{item.title}</Link><p className="mt-1 font-mono text-[0.625rem] uppercase tracking-[0.12em] text-[var(--n100-text-tertiary)]">{item.category}</p></li>)}</ul></div> : null}{hasTokens ? <div><h2 className="text-lg font-semibold text-[var(--n100-text-primary)]">Related Tokens</h2><ul className="mt-4 space-y-3">{article.relatedTokens?.map((token) => { const label = [token.symbol, token.name].filter(Boolean).join(" · ") || "Canonical token identity"; return <li key={`${token.chain}-${token.symbol ?? ""}-${token.contract ?? token.name ?? ""}`} className="text-sm text-[var(--n100-text-secondary)]">{token.href ? <Link href={token.href} className="font-medium text-[var(--n100-text-primary)] underline decoration-[var(--n100-border-strong)] underline-offset-4 hover:text-[var(--n100-accent)]">{label}</Link> : <span className="font-medium text-[var(--n100-text-primary)]">{label}</span>}<p className="mt-1 font-mono text-[0.625rem] uppercase tracking-[0.12em] text-[var(--n100-text-tertiary)]">{token.chain}{token.contract ? ` · ${token.contract}` : ""}</p></li>; })}</ul><p className="mt-4 text-xs leading-5 text-[var(--n100-text-tertiary)]">Related token context is not a buy or sell recommendation.</p></div> : null}</div></Container></Section>;
}
        ...
        <Section className="border-b border-[var(--n100-border-subtle)]">
          <Container size="reading" className="py-16 sm:py-24">
            <header>
              ...
              <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 border-t border-[var(--n100-border-subtle)] pt-5 text-xs text-[var(--n100-text-tertiary)]"><span>By {article.author.name}{article.author.role ? ` · ${article.author.role}` : ""}</span><span>{article.publishedAt ? `Published ${formatDate(article.publishedAt)}` : "Unpublished preview"}</span>{article.updatedAt ? <span>Updated {formatDate(article.updatedAt)}</span> : null}</div>
