...
export function calculatePositionSizing(input: PositionSizingInput): PositionSizingResult {
  try {
    ...
    const rawValues = [riskBudget, ratioToDecimal(stopDistancePercentRatio, OUTPUT_SCALE), ratioToDecimal(positionNotionalRatio, OUTPUT_SCALE)]; const output = rawValues.map((value) => format(value, OUTPUT_SCALE)); if (rawValues.some((value, index) => compare(value, ZERO) !== 0 && output[index] === "0")) return { status: "POSITION_SIZE_UNAVAILABLE", riskBudget: null, stopDistancePercent: null, positionNotional: null, reason: "A nonzero result is below the supported output precision." }; if (output.some((item) => !/^(?:0|\d+(?:\.\d+)?)$/.test(item))) return invalid("The calculation exceeded the supported numeric range.");
