...
function ipv6BigInt(value: string) { const mapped = value.toLowerCase().match(/^::ffff:(\d+\.\d+\.\d+\.\d+)$/); if (mapped) return { mapped: mapped[1], value: null as bigint | null }; const halves = value.toLowerCase().split("::"); if (halves.length > 2) return null; const left = halves[0] ? halves[0].split(":") : []; const right = halves[1] ? halves[1].split(":") : []; const expanded = [...left, ...Array(8 - left.length - right.length).fill("0"), ...right]; if (expanded.length !== 8 || expanded.some((part) => !/^[0-9a-f]{1,4}$/.test(part))) return null; return { mapped: null, value: expanded.reduce((n, part) => (n << BigInt(16)) | BigInt(`0x${part}`), BigInt(0)) };
}
function unsafeAddress(value: string) {
  const stripped = value.replace(/^\[|\]$/g, ""); const family = net.isIP(stripped);
  if (family === 4) return ipv4In(stripped, [0, 0, 0, 0], 8) || ipv4In(stripped, [10, 0, 0, 0], 8) || ipv4In(stripped, [100, 64, 0, 0], 10) || ipv4In(stripped, [127, 0, 0, 0], 8) || ipv4In(stripped, [169, 254, 0, 0], 16) || ipv4In(stripped, [172, 16, 0, 0], 12) || ipv4In(stripped, [192, 0, 0, 0], 24) || ipv4In(stripped, [192, 0, 2, 0], 24) || ipv4In(stripped, [192, 168, 0, 0], 16) || ipv4In(stripped, [198, 18, 0, 0], 15) || ipv4In(stripped, [198, 51, 100, 0], 24) || ipv4In(stripped, [203, 0, 113, 0], 24) || ipv4In(stripped, [224, 0, 0, 0], 4);
