...
import { hasResearchWhitespace, isBlankResearchText, isResearchSourceDate, isResearchSourceUrl, unicodeCodePointLength } from "./text";
...
function text(value: unknown, field: string, max: number, required = false): string | null {
  if (typeof value !== "string" || unicodeCodePointLength(value) > max || /[\u0000-\u001f]/.test(value) || (required && isBlankResearchText(value))) return null;
  return value;
}
...
function source(value: unknown): ResearchInputSource | null {
  ...
  const title = text(item.title, "source title", 240, true); const publisher = text(item.publisher, "publisher", 160, true); const url = text(item.url, "source url", 2048, true);
  if (!title || !publisher || !url) return null;
  if (!isResearchSourceUrl(url)) return null;
  ...
  for (const key of ["published_on", "accessed_on"] as const) { if (item[key] !== undefined && item[key] !== null) { const value = text(item[key], key, 10); if (!value || !isResearchSourceDate(value)) return null; result[key] = value; } }
  return result;
}
...
function block(value: unknown): ResearchAdminBlock | null {
  ...
  const id = item.id === undefined ? undefined : text(item.id, "block id", 80, true); if (item.id !== undefined && (!id || hasResearchWhitespace(id))) return null;
  if (item.type === "data_placeholder") { const label = text(item.label, "data label", 160, true); const description = text(item.description, "data description", 10000, false); return label && description !== null ? { type: "data_placeholder", label, description, ...(id ? { id } : {}) } : null; }
  const body = text(item.text, "block text", 10000, false); if (body === null) return null;
  if (item.type === "paragraph") return { type: "paragraph", text: body, ...(id ? { id } : {}) };
  if (item.type === "quote") { const attribution = item.attribution === undefined ? undefined : text(item.attribution, "attribution", 240, false); return attribution === null ? null : { type: "quote", text: body, ...(attribution ? { attribution } : {}), ...(id ? { id } : {}) }; }
  const label = text(item.label, "callout label", 120, true); return label ? { type: "callout", label, text: body, ...(id ? { id } : {}) } : null;
}
...
function documentBlocks(value: unknown): { version: 1; sections: ResearchInputSection[] } | null {
  ...
  const ids = new Set<string>(); const blockIds = new Set<string>(); let total = 0; const sections: ResearchInputSection[] = [];
  for (const raw of input.sections) { if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null; const item = raw as Record<string, unknown>; const id = text(item.id, "section id", 80, true); const heading = text(item.heading, "section heading", 160, false); if (!id || hasResearchWhitespace(id) || !heading || ids.has(id) || !Array.isArray(item.blocks) || item.blocks.length < 1 || item.blocks.length > 50) return null; ids.add(id); const blocks = item.blocks.map(block); if (blocks.some((entry) => entry === null)) return null; for (const entry of blocks as ResearchAdminBlock[]) { if (entry.id && blockIds.has(entry.id)) return null; if (entry.id) blockIds.add(entry.id); } total += blocks.length; sections.push({ id, heading, blocks: blocks as ResearchAdminBlock[] }); }
...
function facts(value: unknown): ResearchInputFact[] | null {
  ...
  if (typeof serialized !== "string" || new TextEncoder().encode(serialized).length > 16384) return null;
  const result: ResearchInputFact[] = [];
  for (const raw of value) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
    const item = raw as Record<string, unknown>;
    const label = text(item.label, "fact label", 120, true);
    const detail = text(item.detail, "fact detail", 1000, true);
    const kind = item.evidence === undefined ? undefined : item.evidence;
    if (!label || !detail || (kind !== undefined && (typeof kind !== "string" || !evidence.has(kind)))) return null;
    result.push({ label, detail, ...(kind ? { evidence: kind as ResearchInputFact["evidence"] } : {}) });
  }
  return result;
}
