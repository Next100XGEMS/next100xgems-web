...
export class RadarSystemGatewayError extends Error { constructor(message: string, public readonly code?: string) { super(message); this.name = "RadarSystemGatewayError"; } }
...
export type RadarDeepLaneReservation = {
  ...
  taskType: RadarDeepLaneRequest["taskType"];
  methodVersion: string;
  inputVersion: string;
  schemaVersion: RadarDeepLaneRequest["schemaVersion"];
  outputSchemaVersion: RadarDeepLaneRequest["outputSchemaVersion"];
  inputHash: string;
  inputManifest: RadarDeepLaneRequest["inputManifest"];
  evidenceManifest: RadarDeepLaneRequest["evidence"];
  provider: string;
  model: string;
...
async function rpc<T>(name: string, args: Record<string, unknown>) { const { data, error } = await getClient().rpc(name, args); if (error) throw new RadarSystemGatewayError(`Radar system operation ${name} failed.`, error.code); return data as T; }
...
function reservation(row: Record<string, unknown>): RadarDeepLaneReservation { return { requestId: String(row.request_id), requestHash: String(row.logical_request_hash), requestState: String(row.request_state) as RadarDeepLaneReservation["requestState"], attemptId: String(row.attempt_id), attemptNumber: Number(row.attempt_number), attemptState: String(row.attempt_state) as RadarDeepLaneReservation["attemptState"], invocationOwner: row.invocation_owner === true, providerIdempotencyKey: String(row.provider_idempotency_key), pauseGeneration: Number(row.pause_generation), workItemId: String(row.work_item_id), tokenId: String(row.token_id), reservedAnalysisVersion: Number(row.reserved_analysis_version), taskType: String(row.task_type) as RadarDeepLaneReservation["taskType"], methodVersion: String(row.method_version), inputVersion: String(row.input_version), schemaVersion: String(row.schema_version) as RadarDeepLaneReservation["schemaVersion"], outputSchemaVersion: String(row.output_schema_version) as RadarDeepLaneReservation["outputSchemaVersion"], inputHash: String(row.input_hash), inputManifest: Array.isArray(row.input_manifest) ? row.input_manifest as RadarDeepLaneReservation["inputManifest"] : [], evidenceManifest: Array.isArray(row.evidence_manifest) ? row.evidence_manifest as RadarDeepLaneReservation["evidenceManifest"] : [], provider: String(row.provider), model: String(row.model), modelRevision: row.model_revision === null || row.model_revision === undefined ? null : String(row.model_revision), adapterVersion: String(row.adapter_version), completionOutput: row.completion_output ?? null, completionOutputHash: row.completion_output_hash ? String(row.completion_output_hash) : null }; }
...
export function createRadarSystemGateway(): RadarSystemGateway & RadarDeepLaneGateway { return {
  ...
  reserveDeepLaneAttempt: async (i) => reservation(first(await rpc<Record<string, unknown>[]>("radar_system_reserve_deep_lane_attempt", { p_work_item_id: i.request.workItemId, p_worker: i.claimed.worker, p_lease_token: i.claimed.leaseToken, p_lease_generation: i.claimed.leaseGeneration, p_task_type: i.request.taskType, p_method_version: i.request.methodVersion, p_input_version: i.request.inputVersion, p_schema_version: i.request.schemaVersion, p_output_schema_version: i.request.outputSchemaVersion, p_input_manifest: i.request.inputManifest, p_evidence_manifest: i.request.evidence, p_provider: i.request.provider, p_model: i.request.model, p_model_revision: i.request.modelRevision, p_adapter_version: i.request.adapterVersion, p_max_attempts: i.maxAttempts, p_explicit_retry: i.explicitRetry }))),
  completeDeepLaneAttempt: async (i) => { const rows = await rpc<Record<string, unknown>[]>("radar_system_complete_deep_lane_attempt", { p_request_id: i.reservation.requestId, p_attempt_id: i.reservation.attemptId, p_work_item_id: i.request.workItemId, p_worker: i.claimed.worker, p_lease_token: i.claimed.leaseToken, p_lease_generation: i.claimed.leaseGeneration, p_pause_generation: i.reservation.pauseGeneration, p_request_hash: i.request.requestHash, p_attempt_number: i.reservation.attemptNumber, p_output: deepLaneOutputPayload(i.result), p_output_hash: null }); const row = first(rows); return { ...i.result, outputHash: String(row.output_hash) }; },
