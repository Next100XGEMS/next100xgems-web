...
export type RadarDeepLaneRequest = {
  ...
  /** Filled by the authoritative database reservation. */
...
export type RadarDeepLaneResult = { schemaVersion: "deep-lane-output-v1"; provider: string; model: string; modelRevision: string | null; requestHash: string; generatedAt: string; inferences: readonly RadarDeepLaneInference[]; outputHash: string | null };
...
export function prepareDeepLaneRequest(input: { workItemId: string; tokenId: string; reservedAnalysisVersion: number; methodVersion: string; inputVersion: string; inputManifest: readonly RadarDeepLaneInputManifest[]; evidence: readonly RadarDeepLaneEvidenceInput[]; provider: string; model: string; modelRevision?: string; adapterVersion: string; frozenInputHash: string }): RadarDeepLaneRequest {
  ...
  return Object.freeze({ ...base, requestHash: null });
}
...
export function validateDeepLaneOutput(raw: unknown, request: RadarDeepLaneRequest, trustedIdentity?: { provider: string; model: string; modelRevision?: string; adapterVersion?: string }): RadarDeepLaneResult {
  if (!request.requestHash) throw new RadarDeepLaneValidationError("Deep Lane output requires an authoritative database request hash.");
  safeHash(request.requestHash, "requestHash");
...
export async function executeDeepLaneWork(input: { claimed: RadarClaimedWork; request: RadarDeepLaneRequest; adapter: RadarDeepLaneAdapter; gateway: RadarDeepLaneGateway; maxAttempts?: number; explicitRetry?: boolean }): Promise<RadarDeepLaneResult> {
  ...
  if (reservation.requestState === "SUCCEEDED") {
    ...
    return { ...replay, outputHash: reservation.completionOutputHash };
  }
  if (!reservation.invocationOwner) throw new RadarDeepLaneInProgressError();
  let raw: unknown;
  try {
    raw = await input.adapter.request(authoritativeRequest, { requestHash: authoritativeRequest.requestHash, attemptId: reservation.attemptId, attemptNumber: reservation.attemptNumber, providerIdempotencyKey: reservation.providerIdempotencyKey, supportsIdempotencyKey: input.adapter.supportsIdempotencyKey });
  } catch (error) {
  ...
  try { result = validateDeepLaneOutput(raw, authoritativeRequest, trusted); }
  ...
  return input.gateway.completeDeepLaneAttempt({ claimed: input.claimed, request: authoritativeRequest, reservation, result });
}
